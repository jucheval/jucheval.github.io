...
Here is the [link to an interactive simulation](/assets/html/bivariate_hawkes.html) of a bivariate Hawkes process in JavaScript using *Plotly.js*. The intensities are given by: (the colors below correspond to the colors in the animation)

$$
\begin{align*}
{\color{darkblue} \lambda^1_t} &= \max \left(0, \, \mu_1 + \int_0^t \beta_{11} e^{-\alpha(t-s)} {\color{darkblue} N^1}(\mathrm{ds}) + \int_0^t \beta_{12} e^{-\alpha(t-s)} {\color{orange} N^2}(\mathrm{ds}) \right),\\
{\color{orange} \lambda^2_t} &= \max \left(0, \, \mu_2 + \int_0^t \beta_{21} e^{-\alpha(t-s)} {\color{darkblue} N^1}(\mathrm{ds}) + \int_0^t \beta_{22} e^{-\alpha(t-s)} {\color{orange} N^2}(\mathrm{ds}) \right).
\end{align*}
$$
